...
namespace Bridgepay\Bridge\Block\Order;
...
class Info extends \Magento\Framework\View\Element\Template
{
     ...
     * @var string
     */
    protected $_template = 'Bridgepay_Bridge::order/info.phtml';
    ...
    public function getInfo()
    {
        ...
        if (!$info instanceof \Magento\Payment\Model\InfoInterface) {
            $exceptionInfo = __('We cannot retrieve the payment info model object.');
            throw new \Magento\Framework\Exception\LocalizedException($exceptionInfo);
        }
    ...
    {
        $asset = $this->_assetRepo->createAsset('Bridgepay_Bridge::images/logo-order.png');
     ...
     */
    public function getAdditionalBridgePaymentInformations()
    {
        $order = $block->getInfo()->getOrder();
        $informations = $order->getPayment()->getAdditionalInformation();

        if ($informations['Payment Status'] != __('Created')) {
            $html = __('You have chosen to pay your order by bank transfer using Bridge.');
            $html .= __('Your transfer is awaiting receipt.');
    ...
    public function getIsSecureMode()
    {
        if ($this->hasIsSecureMode()) {
            return (bool) $this->_getData('is_secure_mode');
        }
